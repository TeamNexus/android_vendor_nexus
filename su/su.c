...
int access_disabled(const struct su_initiator *from) {
    ...
    data = read_file("/data/property/persist.sys.nexussud.enabled");
    if (data != NULL) {
        len = strlen(data);
	...
	/* enforce persist.sys.nexussud.enabled on non-eng builds for apps */
    if (strcmp("eng", build_type) != 0 &&
            from->uid != AID_SHELL && from->uid != AID_ROOT &&
            (atoi(enabled) & ROOT_ACCESS_ENABLED) != ROOT_ACCESS_ENABLED ) {
